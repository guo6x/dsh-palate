...
var PalateStore = class {
  ...
  searchExamples(subject, { tag, limit = 12 } = {}) {
    ...
    const examples = this.listExamples({ tag, limit: 1e3 });
    const ranked = examples.map((example) => {
      ...
      const score = terms.reduce((sum, token) => sum + (tagTerms.has(token) ? 3 : 1), 0) + (query.length >= 8 && text.includes(query) ? 4 : 0) + (tag && example.tags.includes(tag) ? 2 : 0);
